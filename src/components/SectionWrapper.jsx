        ...
        <div className={`${styles.descDiv} ${reverse ? "fadeRightMini" : "fadeLeftMini" } ${reverse ? styles.textRight: styles.textLeft}`}>
          ...
          {showBtn && (
            <Button
              ...
              link="https://expo.dev/@anonymouskaish1511/React_Native_app?serviceType=classic&distribution=expo-go"
            />
          )}
        </div>
        <div className={`flex-1 ${styles.flexCenter} p-8 sm:px-0`}>
          <img src={mackupImg} alt="muckup" className={`${styles.sectionImg} ${reverse ? "fadeLeftMini" : "fadeRightMini" }`} />
        </div>
      </div>
    </div>
  );
